...
#     loopback),
#   - the same startup-wait preamble (postgres → paperclip → /health).
#
...
# everything else. The local-DB path no longer needs a password file:
# `database.createLocally = true` uses PostgreSQL peer authentication over
# the Unix socket — see `nix/modules/nixos/paperclip.nix` for the wiring.
let
  inherit (pkgs) lib;
  ...
  # For tests that only inspect ExecStartPre artefacts and never need the
  # main process up (e.g. module-grafana.nix, which mocks an unreachable
  # OTLP endpoint). Only block on postgres being up.
...
in
{
  /*
    ...
    (createLocally = false). Set `mode = "preStartupOnly"` for tests
    that intentionally never wait for paperclip.service to come up.

    Default `mode = "local"` matches the createLocally = true happy
    path (PostgreSQL peer auth via the Unix socket).
  */
  mkPaperclipTest =
    {
      name,
